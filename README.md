...
- SMSBower 和 Hero-SMS 的 API Key 初始为空，需要在“系统设置”中自行填写。
- 注册 PIN、登录原 PIN、登录新 PIN、代理池和默认代理区域初始为空。
- 账号、注册与登录日志、任务、支付记录、短信激活记录和管理员账号均不会预置。
- 服务地址、服务代码、国家代码、任务数量、并发数和超时秒数属于功能性默认配置，不是业务数据或访问凭据。
- 本地 SQLite 数据库、数据库密钥、备份、运行日志和浏览器测试产物已由 `.gitignore` 排除，不会随源码提交。
- 首次启动会在 `data/` 下创建新的 SQLite 数据库和 AES-GCM 数据密钥，页面数据全部来自这一本地数据库。
